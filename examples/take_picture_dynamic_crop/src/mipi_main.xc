    ...
    switch (data_type)
    {
        ...
        case EXPECTED_FORMAT:{ // save it in SRAM and increment line
          ...
          if (img_rx->line_number >= MIPI_IMAGE_HEIGHT_PIXELS){
            ...
            if (img_rx->line_number >= Y_START_CROP){
              if (img_rx->line_number < (Y_START_CROP + CROP_HEIGHT_PIXELS)){
                // compute x axis             
                uint32_t pos = ((img_rx->line_number) - Y_START_CROP)* CROP_WIDTH_PIXELS;
                // printf("pos = %d\n", pos);
                c_memcpy(&img_raw_ptr[pos], &pkt->payload[X_START_CROP], CROP_WIDTH_PIXELS); 
              }
            }
          #else
    ...
    chanend_t ch_exp)
{
  ...
  while (1)
  {
        ...
        handle_packet(&img_rx, pkt, image_ptr, ch_exp);
...
void mipi_main(client interface i2c_master_if i2c)
{
  ...
  #if (CROP_ENABLED)
    uint8_t* unsafe img_raw_ptr = malloc(CROP_WIDTH_PIXELS * CROP_HEIGHT_PIXELS * sizeof(uint8_t));
  #else
